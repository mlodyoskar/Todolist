...
export const TodoForm = ({ formValues, handleInputChange, handleSubmit, toggleAddingTodo }) => {
    const [datePopupVisable, setDatePopupVisable] = useState(false);
    const [datePopupValue, setDatePopupValue] = useState(Date.now());
    const datePopupValues = ["Today", "Tommorrow"];

    const togglePopopVisibility = (func) => {
    ...
    return (
        ...
        <Wrapper onSubmit={handleSubmit} >
            <InputWrapper>
                ...
                />
                <ButtonsWrapper>
                    <Button onClick={() => togglePopopVisibility(setDatePopupVisable)} type="button" secondary >
                        <CalendarIcon /> Today
                    </Button>
                    {datePopupVisable ? <Popup items={datePopupValues} /> : null}
